...
class TC:
    ...
    tmo_high_qc_rounds: Set[int]  # A vector of 2f + 1 high qc round numbers of timeout messages that form TC
    # TODO: signatures might change to bytes
    tmo_signatures: Set[str]  # A vector of 2f + 1 validator signatures on (round, respective high qc round)


@dataclass
class TimeoutMsg:
    tmo_info: TimeoutInfo  # TimeoutInfo for some round with a high qc
    last_round_tc: TC  # TC for (tmo_info.round - 1) if tmo_info.high_qc.round != (tmo_info.round - 1), else None
...
class ProposalMsg:
    block: Block
    last_round_tc: TC  # TC for (block.round - 1) if block.qc.vote_info.round != (block.round - 1), else None
